...
// Fecha: 15/09/2026
// Archivo p01_single_grades.cc: programa cliente del primer ejercicio.
// Carga las calificaciones, muestra la máxima por estudiante y permite insertar
// elementos individualmente sin modificar el fichero de entrada.
...
void PrintHelp() {
  ...
  std::cout << "Para cada estudiante se conserva únicamente la nota máxima."
            << std::endl;
 ...
 */
void AddInteractiveGrade(SingleGradeManager& manager) {
  std::string alu;
  double grade;
  ...
  // Añade la calificación al gestor, conservando la máxima para el ALU dado
...
/**
 * @brief Punto de entrada del programa del primer ejercicio.
...
int main(int argc, char* argv[]) {
  // Comprueba si el usuario solicitó ayuda
  ...
  if (argc != 2) {
    // Muestra un mensaje de error indicando el modo de uso correcto
  ...
  // Crea un gestor de calificaciones que mantiene la máxima por estudiante
  SingleGradeManager manager;
  ...
  // Muestra las calificaciones máximas por estudiante tras la lectura del fichero
  manager.Print();
